...
import { MyRequestsComponent } from '../../base/my-requests/my-requests.component';
...
export class WidgetsDropdownComponent implements OnInit, AfterContentInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getAllUsers().subscribe(users => {
      this.users = users;
      this.usersLength = users.length ?? 0;
    });
    this.orgService.getOrganisations().subscribe(orgs => {
      this.organisations = orgs;
      this.orgsLength = orgs.length ?? 0;
    });
    this.requestService.getAllRequests().subscribe(requests => {
      this.requests = requests;
      this.requestsLength = requests.length ?? 0;
    });
    this.requestService.getAllReports().subscribe(reports => {
      this.reports = reports;
      this.reportsLength = reports.length ?? 0;
    });
  }
